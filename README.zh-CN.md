...
| 🔗 原生沉淀到知识库 | 会先按论文领域归档到现有知识库结构，再为每篇论文生成独立文件夹、带 YAML properties 和固定`核心信息`元数据块的 Markdown 笔记、稳定的 `images/` 目录，以及更干净的图表嵌入。 |
...
- 如果你没有配置 Obsidian，它也能把笔记输出到当前工作区下的回退输出目录，默认是 `DeepPaperNote_output`。
